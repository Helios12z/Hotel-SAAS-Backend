...
namespace Hotel_SAAS_Backend.API.Services
{
    public class DashboardService(ApplicationDbContext context) : IDashboardService
    {
        ...
        public async Task<OccupancyStatsDto> GetOccupancyStatsAsync(Guid hotelId)
        {
            ...
            {
                adr = (decimal)monthlyBookings.Average(br => (double)br.Price);
            }
            else
            {
                var avgPrice = await rooms.AverageAsync(r => (decimal?)r.BasePrice);
                adr = avgPrice ?? 0;
            }
        ...
        public async Task<ReviewStatsDto> GetReviewStatsAsync(Guid hotelId)
        {
            ...
            var approvedCount = await approvedReviews.CountAsync();
            float averageRating = 0;
            if (approvedCount > 0)
            {
                averageRating = (float)await approvedReviews.AverageAsync(r => r.Rating);
            }
